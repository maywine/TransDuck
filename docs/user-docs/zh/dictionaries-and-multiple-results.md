...
切换提供商时，当前设置窗口会保留未保存的字段，包括尚未填写完整的值。请分别保存各个
提供商；保存其中一个不会丢弃其他提供商的草稿。草稿仅保留在内存中，关闭设置窗口或在
macOS 上选择 **重新加载并放弃草稿** 后即丢弃。已保存的凭据不会回填到表单。

## 翻译与复制结果

在其他应用中按 **Ctrl+Alt+T**（Windows）或 **Command+Option+T**（macOS），即可打开
手动翻译窗口。输入框会获得焦点并选中原有文本，直接输入即可替换；此操作不会读取剪贴板
或外部选区，也不会自动发起查询。在设置的 **全局快捷键 → 打开翻译窗口** 中修改组合键，
点击 **保存打开窗口快捷键**。它与选中文本翻译快捷键分开保存，两组组合键不能相同。
macOS 的全局快捷键需要辅助功能权限；未授权时仍可从菜单栏打开窗口手动输入。

输入或粘贴文本后点击 **翻译**，也可在 Windows 上按 **Ctrl+Enter**，在 macOS 上按
**Command+Return**；单独按 Enter 换行。操作进行中显示 **取消**，有可重试的失败时显示
**重试失败来源**。

截图 OCR 旁的 **识别语言** 控制文字识别语言，不是翻译目标语言。各提供商的翻译目标语言
在设置中分别配置，查询开始后会显示在对应结果卡片上。

每张结果卡片的 **复制** 按钮只复制该卡片的文本。**复制全部结果** 则合并所有非空卡片，
并包含来源名称。
