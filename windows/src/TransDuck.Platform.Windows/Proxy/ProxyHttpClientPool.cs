...
/// </summary>
public sealed record ProxyHttpTransportOptions(WindowsProxyMode Mode, Uri? CustomHttpProxyUri)
{
    /// <summary>Gets the transport policy that directly connects without a proxy.</summary>
    public static ProxyHttpTransportOptions Direct { get; } = new(WindowsProxyMode.Disabled, null);
...
public sealed class ProxyHttpClientPool : IDisposable
{
    ...
    public ProxyHttpClientPool()
        : this(WindowsProxySettings.Default, handlerFactory: null)
    {
    }
    ...
    public ProxyHttpClientPool(
        WindowsProxySettings initialSettings,
    ...
    public WindowsProxySettings CurrentSettings
    {
        get
        {
            lock (_gate)
    ...
    /// </summary>
    public long Update(WindowsProxySettings settings)
    {
    ...
    private static void ValidateSettings(WindowsProxySettings? settings, string parameterName)
    {
        if (settings is null)
        {
    ...
    internal sealed class Generation : IDisposable
    {
        ...
        public WindowsProxySettings Settings { get; }
    ...
    private sealed class DefaultProxyHttpMessageHandlerFactory : IProxyHttpMessageHandlerFactory
    {
        public HttpMessageHandler Create(ProxyHttpTransportOptions options)
        {
            ...
            {
                WindowsProxyMode.SystemDefault => CreateSystemDefaultHandler(),
                WindowsProxyMode.CustomHttp => CreateCustomHttpHandler(options.CustomHttpProxyUri),
                WindowsProxyMode.Disabled => CreateDirectHandler(),
        ...
        {
            if (!WindowsProxySettings.IsValidCustomHttpProxyUri(proxyUri))
            {
