...
public sealed class ProxyHttpClientPoolTests
{
    ...
    public void Constructor_CreatesSystemDefaultAndDirectHandlersWithoutCredentialOptions()
    {
        ...
        Assert.Equal(ProxySettings.Default, pool.CurrentSettings);
        Assert.Equal(1, pool.CurrentGeneration);
        Assert.Equal(
            new[]
            {
                new ProxyHttpTransportOptions(ProxyMode.SystemDefault, null),
    ...
    private static ProxySettings Disabled() => new(
        ProxySettingsMigration.CurrentVersion,
        ProxyMode.Disabled,
        null);
